test_that("CNAObject", {
  ...
  bins <- keepSeqlevels(bins1kb, "chr3")
  ...
  gc.adji <- as.integer(round(1000*gc_adj, 0))
  ...
  ##devtools::load_all("~/Software/svpackages/svcnvs")
  ##devtools::install_local("~/Software/svpackages/svcnvs")
  dat <- svcnvs:::CNAObject(pview)
  expect_is(dat, "CNA")

  select <- 1:10
  dat2 <- svcnvs:::CNAObject(pview[select, ])
  expect_identical(dat[select, ], dat2)

  bins$gc.adj <- gc.adj
  bins$id <- colnames(pview)[1]
  dat3 <- svcnvs:::CNAObject(bins, "gc.adj")
