...
#'
#' Amplicons selected to seed a graph have a fold change of at least
#' log2(2.75), or nearly 3-fold the diploid genome. Lower-copy
#' amplicons flanking the seed amplicons are pulled into the graph if
#' they have a fold change of at least log2(1.75), or nearly 2-fold
#' the diploid genome.  To assess whether a low-copy amplicon is
#' 'flanking' an amplicon seed, all read pairs that aligned to the
#' amplicon seeds (see \code{get_readpairs}) are passed to this
#' function in object \code{rp}.  The read pairs are converted to
#' segments by \code{readPairsAsSegments} and an assessment of whether
#' flanking duplications are linked is given by
#' \code{linkedDuplicatedRanges}.  Finally, the flanks are added to
#' the graph by the \code{addFlanks} function.
#'
...
#' Identify lower copy focal amplicons not already annotated as
#' amplicons in an AmpliconGraph
#'
#' Select all duplicated segments (see \code{isDuplication}) less than
#' 500kb that are not within a germline CNV or outlier.  Germline CNVs
#' and outliers can be conveniently extracted from the graph object as
#' given in the examples.
#'
#' @export
#'
#' @param object A \code{AmpliconGraph}
#' @param LOW_THR A length-one numeric vector providing the minimum
