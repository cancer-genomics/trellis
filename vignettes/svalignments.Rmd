...
to a `data.table` and grouping the fragments by chromosome, start, end, and
barcode.

```{r unique_fragments}
### Now to data.table
fragments.dt <- data.table(chr = as.character(seqnames(fragments)),
...
uniquefragments <-
    fragments.dt %>%
    group_by(chr, start, end, barcode) %>%
    summarize(n = n()) %>%
    filter(n > 2)
### Get widths
uniquefragments <-
    uniquefragments %>%
    mutate(width = start - end + 1)
```
