---
title: "Identifying discordant read pairs and split reads"
author: "Robert Scharpf"
date: \today
output: BiocStyle::pdf_document
vignette: >
  %\VignetteIndexEntry{Discordant read pairs and split reads}
...
The main use-cases of the alignment utilities in trellis are to identify rearranged
read pairs that support structural variants and to re-align these reads using the local aligner BLAT.
Most use-cases require a BAM file.
For the purpose of illustrating examples in this vignette,
we use BAM files in the `svbams` package.
...
DEPRECATE: duplicates can be marked in a way that is barcode-aware. Downstream
processing would be identical.

```{r targeted_bam, message = F}
