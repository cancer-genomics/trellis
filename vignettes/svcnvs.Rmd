...
This package requires that normalized coverage is calculated for a set of bins in the package `svfilters.hg19` (or `svfilters.hg18`).  See the `svpreprocess` vignette for comprehensive documentation on generating normalized coverage.  This process can take considerable time on large bam files, so for the purpose of this tutorial we've distributed sample preprocessed coverage data.
...
```{r views_filepath}
ddir <- system.file("extdata", package="trellis", mustWork=TRUE)
...
supported by improperly paired reads (hemizygous). 
For identifying somatic deletions without a matched normal, 
we exclude hemizygous deletions not supported by improperly paired reads. 
With this toy dataset, we identify 16 possible homozygous deletions and the calls are both `homozygous` except one which is `homozygous+`.
