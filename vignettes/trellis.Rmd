...
Packages were built using  `R 3.5.0`.

```{r sessionInfo, echo=FALSE}
si <- devtools::session_info()
si$packages <- si$packages[, c("package", "version", "source")]
ix <- grep("local", si$packages$source)
si$packages$source[ix] <- "local"
pacs <- si$packages
colnames(pacs) <- c("R package", "Version", "Source")
pacs %>%
    kable("html") %>%
