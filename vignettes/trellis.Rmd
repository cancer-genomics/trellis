...
df$logr <- threshold(df$logr, ylim, amount=0.4)
...
##df <- rearDataFrameList(rlist3[1:2], build="hg19")
df <- rearDataFrame(rlist3[[1]], build="hg19")
grobs <- ggRearrange(df)
grobs[["arranged.grobs"]]
```

The second rearrangement in the original `rlist` object now corresponds to elements 3 and 4 of the `rlist3` object.  Again, we call `rearDataFrameList` and `ggRearrange` to organize and then plot the supporting reads.
