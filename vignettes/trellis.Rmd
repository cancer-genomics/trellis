...
The trellis package was developed to identify somatic structural variants in tumor-only analyses.  The types of somatic structural alterations that can be identified include deletions, amplifications,, intra-and inter-chromosomal rearrangements, inversions, and in-frame fusions.  Additional packages required for this vignette are the `svfilters.hg19` package that contains various sequence filters for structural variant analyses and the `svbams` package that contains subsampled data that is used to illustrate various aspects of data processing.
...
In this section, we illustrate aspects of the preprocessing that are best handled as a batch job in a non-interactive session.  We begin by extracting all read pairs with aberrant orientation or spacing with respect to the reference genome (improperly paired reads). This step takes a long time depending on the size of the bam file.  In practice, we submit this as a batch job (one sample to each core) and save the result to disk to read in later.
...
To make this toy example run faster, we only preprocess chromosomes 5, 8, and 15.
...
We use infrastructure provided in the Rsamtools package for counting single-end reads overlapping these bins. Bins with zero counts are a consequence of the artificially small bam file included in the `svbams` package and are excluded from further analysis.
...
If one or more normal controls are available, the bin-counts can be further corrected by the bin-wise medians from normals.  This is particularly helpful for capture-based sequencing where bins are often of variable width.  Next, we segment the $\log_2$ ratios using circular binary segmentation (CBS). See `?DNAcopy::segment` for a complete list of parameters available in the `SegmentParam` object.
...
The result is a `GRanges` object with segment means of log-normalized coverage in the `seg.mean` column.   Limited to a very small portion of the genome for illustration, the log ratios obtained from the preceding normalization and GC correction steps can not be interpreted as fold changes from the modal ploidy.  Below, we load log ratios that were obtained from processing the full genome in the same manner illustrated above.
...
g <- readRDS(file.path(path, "cgov44t_segments.rds"))
seqlevels(g, pruning.mode="coarse") <- c("chr5", "chr8", "chr18")
```

The data:

```{r plot, width=10, height=6}
segs.df <- as.tibble(g)
as.tibble(bins) %>%
...
The deletion analysis uses both the improperly paired reads and the normalized read depth.
