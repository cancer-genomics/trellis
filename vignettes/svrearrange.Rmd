---
title: "Overview of rearrangement analyses"
author: "Robert Scharpf"
date: \today
output: BiocStyle::pdf_document
vignette: >
  %\VignetteIndexEntry{Overview of rearrangement analyses}
...
This package is used to identify somatic rearrangements. Required packages:
...
trace(unmapped_read, browser)
unmapped <- unmapped_read(bam.file, query, yield_size=200000)

unmapped2 <- unmapped_read2(bam.file, query, yield_size=200000)
...
```{r rearDataFrameList, fig.cap="Discordant read pairs and split reads supporting a rearrangement.  For the discordant read pairs, the data structure we adopted is a `GAlignmentPairs` object where the 5-prime reads are accessed by the method `first` and the 3-prime reads are accessed by the method `last`.  The shorthfand used to indicate the sequenced strands of the tumor genome is <READ NUMBER><STRAND 5-prime><STRAND 3-prime>.  For example, R1+- denotes a read pair where the first read (R1) is aligned to the 5-prime side of the junction on the positive strand and its mate is aligned to the 3-prime side of the junction on the minus strand.  Read pairs R1+- and R2-+ indicate that the positive strand was sequenced on both sides of the junction.  Read pairs R1-+ and R2+- indicate that the negative strand was sequenced on both sides of the junction.  Hence, this rearrangement did not result from an inversion.  (An inversion would have read pairs R1++ and R2-- or R1-- and R2++.)"}
